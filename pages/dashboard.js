...
const Dashboard = () => {
  ...
  const dashboardData = {
    ...
    departmentDistribution: {
      ...
      Operations:
        employees?.filter((emp) => emp?.department === 4)?.length || 0,
      IT: employees?.filter((emp) => emp?.department === 5)?.length || 0,
    },
    employmentStatus: {
      fulltime:
        employees?.filter((emp) => emp?.employmentStatus === "Full Time")
          ?.length || 0,
      parttime:
        employees?.filter((emp) => emp?.employmentStatus === "Part Time")
          ?.length || 0,
      contract:
        employees?.filter((emp) => emp?.employmentStatus === "Contract")
          ?.length || 0,
      internship:
        employees?.filter((emp) => emp?.employmentStatus === "Internship")
          ?.length || 0,
    },
    genderDistribution: {
  ...
  return (
    <Box>
      <Navbar />
      <Box
        sx={{ padding: { xs: "1rem", md: "1rem 2rem" }, position: "relative" }}
      >
        <Backdrop
          sx={{
            color: "#fff",
                ...
                <Grid item xs={6} sm={6} md={4} key={i}>
                  ...
                  >
                    ...
                    {card.title === "Gender Distribution" ? (
                      <>
                        <Typography fontWeight="bold" fontSize=".8rem" mb={1}>
                          Gender Distribution
                        </Typography>
                        <Grid container spacing={1}>
                          ...
                          >
                            <Man color="primary" />{" "}
                            <Typography>
                              {dashboardData.genderDistribution.male}
                            </Typography>
                          </Grid>
                          <Grid
                            item
                          ...
                          >
                            ...
                            <Typography>
                              {dashboardData.genderDistribution.female}
                            </Typography>
                          </Grid>
                        </Grid>
                      </>
                      ...
                      <>
                        ...
                        <Typography fontSize="13px" mt={0.5} mb={1}>
                          Task
                        </Typography>
                        <Typography color="#0BDA51" fontSize="13px">
                          +35% This Month
                        </Typography>
                      </>
                    )}
